...
2. If the user specifies to ingest the newest files in `_raw/`, get the latest created files
3. If no unprocessed files are found, tell the user

**Only ingest sources that are new.** Grep the log: `grep "filename" _ref/log.md` to check if already ingested.

## Check for Existing Notes

Before creating a new resource note, search the vault for existing notes that cover the same topic:
1. Read `_ref/index.md` and search `resources/` for relevant notes
...
3. **Tag the topic**
   Check the registered tags in `_ref/SCHEMA.md`. Reuse existing tags that are  appropriate for the topic or register new tags if needed.

4. **Create or locate the note**
   - Determine the appropriate category folder under `resources/`; create it if it doesn't exist
   - Create a new file in `resources/<category>/` using the template at `_ref/templates/resource.md` and the schema defined at `_ref/SCHEMA.md`, named after the source (slugified).  
...
8. **Update the index**
   Add or update the entry in `_ref/index.md` under the correct category heading. Follow the format defined in AGENTS.md section "Indexing".
...
11. **Add to log**
    Append an `ingest` entry to `_ref/log.md`:
    ```
