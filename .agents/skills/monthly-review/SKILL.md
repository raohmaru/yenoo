...
2. Check if this month's summary exists at `daily/YYYY/MM/monthly.md`:
   - **If not:** create it from the template at `_ref/templates/monthly.md` (if the template is missing, create a minimal note with the standard sections and warn the user).
...
4. Extract:
   ...
   - New notes created in `projects/` and `areas/` (check `_ref/index.md` for latest activity)
5. Check:
...
7. Generate or update the monthly summary note at `daily/YYYY/MM/monthly.md` using the template at `_ref/templates/monthly.md`. Update the frontmatter `title`, `created`, and `updated` dates to match the current month.
8. Update `_ref/index.md` with any new or changed notes.
9. Ask: "Any wins or lessons you want to add before I save?"
10. Append a `monthly-review` entry to `_ref/log.md` summarizing in one short line what was archived, any reclassifications, and key patterns found.
