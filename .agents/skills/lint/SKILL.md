...
In the `areas/`, `projects/`, `resources/`, `archive/` folders check for:
...
**How to check:**
- Glob all `.md` files in `areas/`, `projects/`, `resources/` and `archive/`
...
**How to check:**
- Grep for `\[\[.*?\]\]` across all `.md` files in `areas/`, `projects/`, `resources/` and `archive/`
...
Every note in in `areas/`, `projects/`, `resources/` and `archive/` should have: title, created, updated, type, status, tags.
...
Append a `lint` entry to `_ref/log.md`:
```
