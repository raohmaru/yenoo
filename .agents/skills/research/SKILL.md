...
7. **Save** to `resources/<category>/topic-name.md` using the template at `_ref/templates/resource.md` and the schema defined at `_ref/SCHEMA.md`. Reuse existing category subfolder or create a new one.  
...
12. **Update `_ref/index.md`** — add a new entry if the note was created, or update the existing entry's summary and date if modified.
...
15. **Append** a `research` entry to `_ref/log.md` summarizing in one short line what was researched.
