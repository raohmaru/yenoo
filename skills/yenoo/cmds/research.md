...
1. **Search the web** for current information about the topic:
  - Break it into 3-5 specific sub-topics worth researching
  - Search each one independently using varied phrasing. Use `WebFetch` (or `defuddle <url>` if available — cleaner extraction) to get content.
  - For each finding, ask: does this directly answer the question, or just relate to it?
  - Keep only what directly answers. Discard the rest.
...
4. **Synthesize** into a structured note:
   - **Overview** (what it is)
   - **Key Concepts** (the important parts)
   - **Connections** (links to existing vault notes)
   - **Sources** (source URLs)

5. **Ensure the directory** `resources/<category>/` exists (create if missing) after determining the appropriate category folder.

6. **Save** to `resources/<category>/topic-name.md` using the template at `ref/templates/resource.md` and the schema defined at `ref/SCHEMA.md`. Reuse existing category subfolder or create a new one.  
   If the template is missing, create a minimal note with the standard sections and warn the user.

7. **Strip sensitive data**: API keys, tokens, passwords, anything marked private.

8. **Add [[wikilinks]]** to related notes already in the vault.

0. **Self-review**: Is the note well-structured? Does it cite sources? Is it consistent with the rest of the vault? Correct as needed.

10. **Bump the `updated` date** in frontmatter to today.

11. **Update `index.md`** — add a new entry if the note was created, or update the existing entry's summary and date if modified. Follow the format defined in SKILL.md section "Indexing".

12. **Ask** if a new project or area should be created or updated out of the topic. If yes, create the relevant note from its template and classify it.

13. **Report** results to the user: what was created or updated, any new projects/areas, and any contradictions found.

14. **Insert** a `research` entry to `log.md` summarizing in one short line what was researched.
